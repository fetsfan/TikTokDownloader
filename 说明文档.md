...
- 2025-11-08 部署健康检查优化（修复根路径造成的失败判断）
  - 根路径 `/`：由“重定向至 GitHub”改为返回 `200` JSON（包含 `docs` 与 `repository` 链接），避免 PaaS 将 3xx 视为失败。
  - 新增健康检查端点：`GET /health` 返回 `200 OK`，不需要令牌，用于部署平台健康检查。
  - Zeabur 建议配置：
    - Service Port：`5555`；如平台需显式设置 `PORT`，环境变量 `PORT=5555`。
    - Health Check Path：`/health`（不要用 `/` 或外链重定向）。
    - 环境变量：`DOUK_TOKEN=<强随机值>`（公开部署强烈建议）。
    - 存储挂载：务必挂载到 `/app/Volume`。

- 2025-11-08 Docker 内置健康检查（HEALTHCHECK）
  - 新增 `/app/healthcheck.py`，使用 Python 标准库请求 `http://localhost:<PORT>/health`。
  - Dockerfile 增加：
    - `COPY healthcheck.py /app/healthcheck.py`
    - `HEALTHCHECK --interval=10s --timeout=3s --start-period=40s --retries=3 CMD python /app/healthcheck.py || exit 1`
  - 作用：容器自检，便于 Zeabur/本地 Docker 统一判断服务就绪。
  - 注意：`PORT` 默认 `5555`；如在平台设置了不同端口，请同步设置环境变量 `PORT`。
